...
async def test_live_capture_service_reads_incremental_current_session() -> None:
    ...
    started = await service.start(adopt_existing=True)
    fake_client.current_export = [
        _entry(path="/baseline"),
        _entry(path="/live", start="2026-03-06T10:00:01.000+08:00"),
    ]

    first = await service.read(started.capture_id, cursor=0)
    fake_client.current_export.append(_entry(path="/later", start="2026-03-06T10:00:02.000+08:00"))
    second = await service.read(started.capture_id, cursor=first.next_cursor)

    assert "history" not in fake_client.calls
    assert [item["path"] for item in first.items] == ["/baseline", "/live"]
    assert [item["path"] for item in second.items] == ["/later"]
