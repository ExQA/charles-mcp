...
@pytest.mark.asyncio
async def test_vnext_start_live_analysis_passes_the_snapshot_format(monkeypatch) -> None:
    captured: dict[str, object] = {}

    async def fake_start(self, **kwargs):
        captured.update(kwargs)
        return {"live_session_id": "live-test", "snapshot_format": kwargs["snapshot_format"].value}

    monkeypatch.setattr(server_module.LiveAnalysisService, "start", fake_start)

    server = create_server()
    result = _tool_result(await server.call_tool("reverse_start_live_analysis", {"snapshot_format": "native"}))

    assert captured["snapshot_format"] is CaptureSourceFormat.NATIVE
    assert result == {"live_session_id": "live-test", "snapshot_format": "native"}


@pytest.mark.asyncio
async def test_vnext_start_live_analysis_rejects_the_removed_summary_format() -> None:
    server = create_server()

    with pytest.raises(Exception, match="must be `xml` or `native`"):
        await server.call_tool("reverse_start_live_analysis", {"snapshot_format": "summary"})
