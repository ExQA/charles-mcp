...
async def test_query_live_capture_entries_prefers_export_and_exposes_next_cursor(
    ...
    assert [item["path"] for item in result["items"]] == ["/api/bootstrap", "/api/profile"]
...
async def test_query_live_capture_entries_honors_live_scan_limit_window(
    ...
    # The recorded /api/bootstrap is part of the capture, the 60 images are not API calls.
    assert result["total_items"] == 62
    assert result["scanned_count"] == 62
    assert result["matched_count"] == 2
    assert [item["path"] for item in result["items"]] == ["/api/bootstrap", "/api/tail"]
