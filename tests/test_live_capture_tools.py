...
async def test_read_live_capture_never_loads_latest_file(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert [item["path"] for item in result["items"]] == ["/baseline", "/new"]
...
@pytest.mark.asyncio
async def test_start_live_capture_always_includes_the_recorded_session(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Traffic recorded before the start is always visible; there is no opt-out."""
    fake_client = _fake_client_class()
    fake_client.current_export = [_entry(path="/recorded-before-start")]
    monkeypatch.setattr(server_module, "CharlesClient", fake_client)
    server = create_server()

    tools = {tool.name: tool for tool in await server.list_tools()}
    assert "include_existing" not in tools["start_live_capture"].inputSchema["properties"]

    started = _tool_result(await server.call_tool("start_live_capture", {}))
    assert started["include_existing"] is True
    capture_id = started["capture_id"]

    queried = _tool_result(await server.call_tool(
        "query_live_capture_entries",
        {"capture_id": capture_id, "preset": "all_http"},
    ))
    assert [item["path"] for item in queried["items"]] == ["/recorded-before-start"]

    first = _tool_result(await server.call_tool("read_live_capture", {"capture_id": capture_id}))
    assert [item["path"] for item in first["items"]] == ["/recorded-before-start"]

    fake_client.current_export = [
        _entry(path="/recorded-before-start"),
        _entry(path="/after-start", start="2026-03-06T10:00:01.000+08:00"),
    ]
    second = _tool_result(await server.call_tool("read_live_capture", {"capture_id": capture_id}))
    assert [item["path"] for item in second["items"]] == ["/after-start"]
