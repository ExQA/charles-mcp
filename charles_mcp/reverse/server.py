...
def _coerce_live_snapshot_format(snapshot_format: str | CaptureSourceFormat) -> CaptureSourceFormat:
    ...
    return CaptureSourceFormat(normalized)
...
def register_reverse_tools(mcp: FastMCP, runtime: ReverseRuntime) -> None:
    ...
    async def reverse_start_live_analysis(
        ...
        return await live_service.start(
            ...
            snapshot_format=_coerce_live_snapshot_format(snapshot_format),
        )
    ...
    async def reverse_analyze_live_login_flow(
        ...
        return await workflow_service.analyze_live_login_flow(
            live_session_id=live_session_id,
            snapshot_format=_coerce_live_snapshot_format(snapshot_format),
            host_contains=host_contains,
    ...
    async def reverse_analyze_live_api_flow(
        ...
        Read summary/report first and expand evidence only as needed."""
        return await workflow_service.analyze_live_api_flow(
            live_session_id=live_session_id,
            snapshot_format=_coerce_live_snapshot_format(snapshot_format),
            host_contains=host_contains,
            path_keywords=path_keywords,
    ...
    async def reverse_analyze_live_signature_flow(
        ...
        Read summary/report first and expand evidence only as needed."""
        return await workflow_service.analyze_live_signature_flow(
            live_session_id=live_session_id,
            snapshot_format=_coerce_live_snapshot_format(snapshot_format),
            host_contains=host_contains,
            path_keywords=path_keywords,
