...
class Config:
    ...
    reverse_live_session_ttl_seconds: int = field(
        default_factory=lambda: _env_int("CHARLES_REVERSE_LIVE_SESSION_TTL_SECONDS", 900)
    )
    reverse_live_snapshot_history_limit: int = field(
        default_factory=lambda: _env_int("CHARLES_REVERSE_LIVE_SNAPSHOT_HISTORY_LIMIT", 20)
    )
    reverse_max_query_limit: int = field(
        default_factory=lambda: _env_int("CHARLES_REVERSE_MAX_QUERY_LIMIT", 100)
    )
    # Root directory of the Map Local mock store. Charles maps
    # https://<host>/* to <mock_dir>/<host>/ and serves files found there.
    mock_dir: str = field(
        default_factory=lambda: os.path.expanduser(
            os.getenv("CHARLES_MOCK_DIR", "~/charles-mocks")
        )
    )
    # Local body-aware dispatcher that Charles Map Remote targets.
    dispatcher_port: int = field(
        default_factory=lambda: _env_int("CHARLES_DISPATCHER_PORT", 18080)
    )
    dispatcher_timeout_seconds: float = field(
        default_factory=lambda: _env_float("CHARLES_DISPATCHER_TIMEOUT", 20.0)
    )
    ...
    def __post_init__(self) -> None:
        ...
        # Captures and Charles config backups hold traffic, tokens, the Charles
        # license key and Web Interface credentials. Keep them in the per-user
        # state dir even when running from a checkout, so archiving or sharing
        # the project folder never takes them along.
        prefer_base_dir = explicit_base_dir
    ...
    def _detect_charles_config_path(self) -> str | None:
        ...
        if sys.platform == "win32":
            ...
            possible_patterns.extend(
                [
                    # Charles 5 on macOS keeps its live settings here.
                    str(user_home / "Library" / "Preferences" / "com.xk72.charles.config"),
                    str(
                        user_home
                        / "Library"
                        / "Application Support"
