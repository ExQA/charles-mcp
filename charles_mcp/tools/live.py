...
def register_live_tools(mcp: FastMCP) -> None:
    ...
    async def start_live_capture(
        ...
        PREFER THIS TOOL when the user wants to inspect ongoing / just-now traffic.
        ...
        is recording (start_recording is idempotent). The capture always
        includes the traffic recorded before this call, so query and read tools
        see the whole session: the first read_live_capture returns the existing
        entries, later reads return only new ones.

        Pass reset_session=true ONLY when the user explicitly asks to clear /
        wipe the current Charles session before starting a fresh
    ...
    async def query_live_capture_entries(
        ...
        This is the RECOMMENDED tool for inspecting live / ongoing traffic.
