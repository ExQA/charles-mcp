    ...
    def _resolve_recording_path(self, path: str) -> str:
        ...
        if not candidate.is_absolute():
            if candidate.parts and candidate.parts[0] == package_dir.name:
                # "package/x.chlsj" is relative to wherever the package dir lives
                # (the user state dir by default), not to the source checkout.
                candidate = package_dir.parent / candidate
            else:
