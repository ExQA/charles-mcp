...
### Переконайтеся, що встановлено потрібну збірку

```bash
uv run --project <PATH> charles-mcp --selftest      # або: <PATH>/.venv/bin/python <PATH>/charles-mcp-server.py --selftest
```

Команда друкує версію і кількість інструментів та завершується словом `OK`.
`FAIL`, нуль інструментів моків або відмова розпізнати `--selftest` означають,
що замість цього форка стоїть пакет upstream із PyPI.
