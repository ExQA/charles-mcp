...
| `mock_dispatcher` | `start` runs the dispatcher on `127.0.0.1` inside the MCP server (it stops with it) and, with `toggle_map_remote=true` (default), enables Charles Map Remote once something answers on the port; `stop` stops that instance and disables Map Remote; `status` reports whether anything answers on the port. Rules, fixtures and the stored mappings stay |
