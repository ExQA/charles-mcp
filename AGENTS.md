...
5. Never restructure a mocked response: keep key order, nesting, wrappers, field names, array order and scalar types from the captured entry, and change only the fields the user named. Apps parse more strictly than JSON requires.
6. Charles loads Map Remote mappings only at startup, so a mapping written with `apply=true` is inert until the user starts Charles; `mock_dispatcher(action="verify")` proves the path end to end.
7. Verify every mock against fresh traffic: `X-Charles-Map-Local` for Map Local, `X-Charles-MCP-Rule` for dispatcher rules (`passthrough` means no rule matched).
8. Remove or disable mocks when the scenario ends; tell the user which Charles mappings stay active and that the dispatcher must run while they do.
